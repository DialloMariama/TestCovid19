...
function validerNom($nom) {
    if (strlen($nom) >= 2 && strlen($nom) <= 40) {
        return preg_match("/^[A-Za-zÀ-ÖØ-öø-ÿ\s'-]+$/", $nom);
    } else {
        return false;
    }
}
...
function validerPrenom($prenom) {
    if (strlen($prenom) >= 3 && strlen($prenom) <= 50) {
        preg_match("/^[A-Za-zÀ-ÖØ-öø-ÿ\s'-]+$/", $prenom);
    } else {
        return false;
    }
}
...
function validerTemperature($temperature) {
    return $temperature >= 30 && $temperature <= 45;
}
function validerPoids($poids) {
    return $poids >= 2 && $poids <= 300;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!validerNom($nom)) {
        array_push($erreurs, "Entrez un nom valide.");
    }

    if (!validerPrenom($prenom)) {
        array_push($erreurs, "Entrez un prénom valide.");
    }

    if (!validerTemperature($temperature)) {
        array_push($erreurs, "Entrez une valeur valide pour la température.");
    }

    if (!validerPoids($poids)) {
        array_push($erreurs, "Entrez une valeur valide pour le poids.");
    }
    if (!validerChampsRadio($maux_tete, $diarrhee, $perte_odorat)) {
        array_push($erreurs, "Les champs radios sont obligatoires.");
    } else {
        switch ($maux_tete) {
            case 'oui':
                $score += 15;
...
<body>
    <form action="index.php" method="post">
        ...
        <input type="number" id="temperature" name="temperature" step="0.01" value="<?php echo $temperature; ?>" min="30" max="45" required >
 
        <label for="poids">Poids (kg)</label>
        <input type="number" id="poids" name="poids" step="0.01" value="<?php echo $poids; ?>" min="2" max="300" required >
        
        <label for="maux_tete">Avez-vous des maux de tête ?</label>
        <input type="radio" id="maux_tete_oui" name="maux_tete" value="oui" <?php if ($maux_tete === 'oui') echo 'checked'; ?> required >
        <label for="maux_tete_oui">Oui</label>
        <input type="radio" id="maux_tete_non" name="maux_tete" value="non" <?php if ($maux_tete === 'non') echo 'checked'; ?> required >
        <label for="maux_tete_non">Non</label>
        
        <label for="diarrhee">Avez-vous la diarrhée</label>
        <input type="radio" id="diarrhee_oui" name="diarrhee" value="oui" <?php if ($diarrhee === 'oui') echo 'checked'; ?> required >
        <label for="diarrhee_oui">Oui</label>
        <input type="radio" id="diarrhee_non" name="diarrhee" value="non" <?php if ($diarrhee === 'non') echo 'checked'; ?> required >
        <label for="diarrhee_non">Non</label>

        <label for="perte_odorat">Avez-vous une perte d'odeur</label>
        <input type="radio" id="perte_odorat_oui" name="perte_odorat" value="oui"  <?php if ($perte_odorat === 'oui') echo 'checked'; ?> required>
        <label for="perte_odorat_oui">Oui</label>
        <input type="radio" id="perte_odorat_non" name="perte_odorat" value="non" <?php if ($perte_odorat === 'non') echo 'checked'; ?> required >
        <label for="perte_odorat_non">Non</label>
            
        <label for="age">Âge</label>
        <input type="radio" id="age_0_14" name="age" value="0-14" <?php if (isset($age) && $age === '0-14') echo 'checked'; ?> required >
        <label for="age_0_14">0-14 ans</label>

        <input type="radio" id="age_15_40" name="age" value="15-40" <?php if (isset($age) && $age === '15-40') echo 'checked'; ?> required >
        <label for="age_15_40">15-40 ans</label>

        <input type="radio" id="age_41_100" name="age" value="41-100" <?php if (isset($age) && $age === '41-100') echo 'checked'; ?> required >
